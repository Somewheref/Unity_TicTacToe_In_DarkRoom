   ...
   public static class Builder
   {
      ...
      private static bool buildPlayer(BuildTarget target, string name, string path, params string[] scenes) //, bool quit = false)
      {
         ...
         if (Helper.hasActiveScenes || scenes?.Length > 0)
         {
            ...
            string dirName = $"{(Config.ADD_NAME_TO_PATH ? $"{name}_" : string.Empty)}{(Config.ADD_VERSION_TO_PATH ? $"{version}_" : string.Empty)}{modifier}{(Config.BO_DEVELOPMENT ? $"_{Constants.TEXT_DEVELOPMENT}" : string.Empty)}{(Config.ADD_DATE_TO_PATH ? $"_{System.DateTime.Now.ToString(Config.DATE_FORMAT)}" : string.Empty)}";
            string buildPath = FileHelper.ValidatePath($"{path}{dirName}");
            ...
            if (deleteBuild)
            {
               try
               {
                  if (Crosstales.Common.Util.FileHelper.ExistsDirectory(buildPath))
                     Crosstales.Common.Util.FileHelper.DeleteDirectory(buildPath);
               }
            ...
            BuildOptions buildOptions = (Config.BO_SHOW_BUILT_PLAYER ? BuildOptions.ShowBuiltPlayer : BuildOptions.None) |
                                        (Config.BO_DEVELOPMENT ? BuildOptions.Development : BuildOptions.None) |
                                        (Config.BO_PROFILER ? BuildOptions.ConnectWithProfiler : BuildOptions.None) |
                                        (Config.BO_DATA_COMPRESSION == CompressionData.LZ4 ? BuildOptions.CompressWithLz4 : Config.BO_DATA_COMPRESSION == CompressionData.LZ4HC ? BuildOptions.CompressWithLz4HC : BuildOptions.None) |
#if UNITY_2020_1_OR_NEWER
                                         (Config.BO_SCRIPTDEBUG ? BuildOptions.AllowDebugging : BuildOptions.None) |
                                         (Config.BO_DETAILED_BUILD_REPORT ? BuildOptions.DetailedBuildReport : BuildOptions.None);
#else
                                        (Config.BO_SCRIPTDEBUG ? BuildOptions.AllowDebugging : BuildOptions.None);
#endif
            UnityEditor.Build.Reporting.BuildReport report = BuildPipeline.BuildPlayer(scenes?.Length > 0 ? scenes : Helper.ScenePaths, playerPath, target, buildOptions);

            if (report.summary.result == UnityEditor.Build.Reporting.BuildResult.Succeeded)
            {
               if (Config.BO_COMPRESS)
                  CompressDirectory(buildPath, $"{path}/{dirName}.zip");
